        ...
        <section className="py-12 max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 ">
            ...
            >
                <CarouselContent className={`-ml-7 md:px-6 ${hasCheckin && 'lg:-ml-2 md:px-0'}`}>
                    {items.map((item) => (
                        <CarouselItem key={item.id} className={`pl-7 md:basis-1/2 lg:basis-1/3 xl:basis-1/4 ${hasCheckin && 'lg:pl-2'}`}>
                            <div className="bg-white rounded-lg overflow-hidden h-full flex flex-col p-2">
                                {/* Image */}
                                <Link href='#' className={`rounded-lg relative h-48 w-full overflow-hidden ${hasCheckin && 'h-70'}`}>
                                    <Image
                                        src={item.image}
                                        alt={item.title}
                                        fill
                                ...
                                <div className="py-4 flex justify-between flex-col grow">
                                    <Link href='#' className={`font-semibold text-base mb-3 grow line-clamp-2 ${hasCheckin && 'text-center'} `}>
                                        {item.title}
                                    </Link>
                ...
                <CarouselPrevious className=" -translate-y-1/2 bg-white rounded-full shadow-lg hover:bg-white/70 transition-colors
                absolute left-0 h-8 w-8 rounded-full border-0 text-black hover:scale-110 hover:shadow-xl transition-all duration-300" />
                <CarouselNext className="-translate-y-1/2 bg-white/80 rounded-full shadow-lg hover:bg-white/70 transition-colors
